...
export class HeadingMetadataComponent implements OnInit, OnDestroy {
  ...
  private setupFormSubscription(): void {
    this.formSubscription = this.headingForm.valueChanges.subscribe(formValues => {
  ...
  private updateErrors(): void {
    ...
    const valueControl = this.headingForm.get('value');
    ...
    const descriptionControl = this.headingForm.get('description');
  ...
  onNameFocusOut(): void {
    const nameValue = this.headingForm.get('name')?.value;
    if (!nameValue || nameValue.trim() === '') {
      // If name is empty, set it to displayName
      this.headingForm.patchValue({ name: this.c.displayName });
    }
  ...
  isFieldInvalid(fieldName: string): boolean {
    const field = this.headingForm.get(fieldName);
