...
export class DateOnlyMetadataComponent implements OnInit, OnDestroy, AfterViewChecked {
  @Input('component') c!: DateOnlyMetadata;
  @ViewChild('nameInput') nameInputRef?: ElementRef;
  showNameInput: boolean = false;
  dateOnlyForm!: FormGroup;
  ...
  private initializeForm(): void {
    this.dateOnlyForm = this.fb.group({
      name: [this.c.name || ''],
  ...
  private setupFormSubscription(): void {
    this.formSubscription = this.dateOnlyForm.valueChanges.subscribe(value => {
  ...
  private updateErrors(): void {
    ...
    if (this.dateOnlyForm.errors) {
      Object.keys(this.dateOnlyForm.errors).forEach(errorKey => {
        this.c.errors!.push({
          field: 'form',
          message: this.getErrorMessage(errorKey, this.dateOnlyForm.errors![errorKey]),
          type: 'custom'
        });
      });
    }
    ...
    Object.keys(this.dateOnlyForm.controls).forEach(controlName => {
      const control = this.dateOnlyForm.get(controlName);
  ...
  private validateDateRange(): void {
    const minDate = this.dateOnlyForm.get('minDate')?.value;
    const maxDate = this.dateOnlyForm.get('maxDate')?.value;
  ...
  isFieldInvalid(fieldName: string): boolean {
    const field = this.dateOnlyForm.get(fieldName);
