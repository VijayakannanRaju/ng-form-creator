...
export class CheckboxMultiMetadataComponent implements OnInit, OnDestroy, AfterViewChecked {
  @Input('component') c!: CheckboxMultiMetadata;
  @ViewChild('nameInput') nameInputRef?: ElementRef;
  showNameInput: boolean = false;
  checkboxMultiForm!: FormGroup;
  ...
  private initializeForm(): void {
    this.checkboxMultiForm = this.fb.group({
      name: [this.c.name || ''],
  ...
  private initializeOptionsArray(): void {
    const optionsArray = this.checkboxMultiForm.get('options') as FormArray;
    optionsArray.clear();
  ...
  get optionsArray(): FormArray {
    return this.checkboxMultiForm.get('options') as FormArray;
  }
  ...
  private setupFormSubscription(): void {
    this.formSubscription = this.checkboxMultiForm.valueChanges.subscribe(value => {
  ...
  private updateErrors(): void {
    ...
    if (this.checkboxMultiForm.errors) {
      Object.keys(this.checkboxMultiForm.errors).forEach(errorKey => {
        this.c.errors!.push({
          field: 'form',
          message: this.getErrorMessage(errorKey, this.checkboxMultiForm.errors![errorKey]),
          type: 'custom'
        });
      });
    }
    ...
    Object.keys(this.checkboxMultiForm.controls).forEach(controlName => {
      const control = this.checkboxMultiForm.get(controlName);
  ...
  isFieldInvalid(fieldName: string): boolean {
    const field = this.checkboxMultiForm.get(fieldName);
